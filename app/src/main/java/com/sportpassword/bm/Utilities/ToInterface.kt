...
//                mainDelegate.startActivity(i)
    ...
    fun toArena(member_like: Boolean = false, isPrevIconShow: Boolean = false, isSearchIconShow: Boolean = false) {
        val i = Intent(mainDelegate, ReadActivity::class.java)
...
//    }
    ...
    fun toSelectWeekdays(selecteds: Int=0, delegate: BaseActivity?=null, able_type: String?=null) {
        ...
        i.putExtra("selected1", selecteds)
...
//        mainDelegate.startActivityForResult(intent, mainDelegate.VALIDATE_REQUEST_CODE)
    ...
    fun toWebView(token: String?, delegate: BaseActivity?) {
        val i = Intent(mainDelegate, WebViewVC::class.java)
        i.putExtra("token", token)

        mainDelegate.delegate = delegate
        mainDelegate.webViewVC.launch(i)
    }
